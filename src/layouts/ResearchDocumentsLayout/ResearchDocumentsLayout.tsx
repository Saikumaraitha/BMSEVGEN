...
import { useLocation, useNavigate, useParams, useSearchParams, useMatch } from 'react-router-dom';
...
import { ResearchDocumentsProvider, useResearchDocumentsContext } from '../../contexts/ResearchDocumentsContext';
...
function LayoutContent({ assetName = 'Asset', lastUpdated, onBack, children }: ResearchDocumentsLayoutProps) {
  ...
  const { docCounts, noteCount, commentCount, hasUnsavedChanges, setHasUnsavedChanges, onSaveAndExit } = useResearchDocumentsContext();
  ...
  const docsSectionItems = RD_LIST_SIDEBAR_ITEMS.filter((i) => i.section === 'docs');
  const sharedSectionItems = RD_LIST_SIDEBAR_ITEMS.filter((i) => i.section === 'shared');
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-1 overflow-hidden">
        <ResearchWorkspaceSidebar collapsed={sidebarCollapsed} onToggle={handleSidebarToggle}>
          {!isWorkspace ? (
            ...
            <nav className="flex flex-col py-2 flex-1">
              ...
              {[
                { key: 'evgen', label: 'Notes', Icon: NoteIcon, count: noteCount, panelParam: '' },
                { key: 'comments', label: 'Comments', Icon: CommentIcon, count: commentCount, panelParam: 'comments' },
              ].map((item) => {
                const isActive = item.key === 'comments' ? activePanel === 'comments' : activePanel !== 'comments';
                return (
                  <button
                    key={item.key}
                    type="button"
              ...
              {[
                { label: 'Add Note', Icon: NoteIcon, onClick: handleAddNoteAction, requiresNotes: false, danger: false },
                { label: 'Generate Gaps', Icon: GapIcon, onClick: handleGenerateGaps, requiresNotes: true, danger: false },
                { label: 'Share Document', Icon: ShareIcon, onClick: handleShareAction, requiresNotes: true, danger: false },
                { label: 'Delete Document', Icon: TrashIcon, onClick: handleDeleteAction, requiresNotes: false, danger: true },
              ].map((item) => {
