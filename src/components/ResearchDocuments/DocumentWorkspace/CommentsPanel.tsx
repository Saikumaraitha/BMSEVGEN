...
interface CommentsPanelProps {
  ...
  onReply: (parentId: string, content: string, author: string, initials: string, noteId?: string) => void;
  onClearFilter: () => void;
}
...
function CommentsPanel({ comments, filterNoteId, filterNoteTitle, onPost, onReply, onClearFilter }: CommentsPanelProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-tab-bg overflow-hidden">
      ...
      {filterNoteId && (
        <div className="flex items-center gap-2 px-4 py-2 bg-primary-tint-04 border-b border-rd-card-border justify-end flex-shrink-0 ">
      ...
      {!filterNoteId && comments.length === 0 ? (
        <div className="flex flex-col items-center justify-center flex-1 gap-3 px-4 text-center">
          <i className="bi bi-chat-text text-4xl text-neutral-300" aria-hidden="true" />
          <div className="flex flex-col gap-1">
            <p className="text-sm font-semibold text-neutral-600">Select a note to view or add comments</p>
            <p className="text-xs text-neutral-400">Comments are linked to individual notes</p>
          </div>
        </div>
      ) : (
        <>
              ...
              {visibleGroups.length === 0 ? (
                ...
                visibleGroups.map(([noteId, { noteTitle, items }]) => (
                  ...
                  >
                    ...
                    <div className="px-4">
                      {items.map((comment) => (
                        <CommentThread key={comment.id} comment={comment} onReply={(parentId, content, author, initials) => onReply(parentId, content, author, initials, noteId)} />
                      ))}
                    </div>
                  </div>
                ))
          ...
          <div className="flex-shrink-0 px-4 py-4 border-t border-neutral-200 bg-white">
            <textarea
              value={postText}
              onChange={handlePostChange}
              placeholder={filterNoteId ? "Leave a comment" : "Select a note to leave a comment"}
              rows={3}
              disabled={!filterNoteId}
              className="w-full border border-neutral-200 rounded-xl px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-brand-primary bg-white disabled:bg-neutral-50 disabled:cursor-not-allowed"
            />
            <div className="flex items-center justify-end gap-2 mt-2">
              <button
                ...
                disabled={!filterNoteId}
                className="px-4 py-1.5 rounded-lg text-sm font-medium border border-neutral-200 text-neutral-600 hover:bg-neutral-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Cancel
              </button>
              <button
                ...
                disabled={!postText.trim() || !filterNoteId}
